package com.kdm.cache.service;

import com.kdm.cache.bean.Employee;
import com.kdm.cache.mapper.EmployeeMapper;
...
public class EmployeeService {
     ...
     *                  condition = "#id>0"
     *                  condition = "#a0>1"：第一个参数的值 > 1的时候才进行缓存(默认第一个参数为a0)
     ...
     *
     *   4、给容器中注册了CacheManager：ConcurrentMapCacheManager
     *   5、可以获取创建ConcurrentMapCache类型的缓存组件：他的作用将数据保存在ConcurrentMap中；
     *
     *   运行流程：
     *   @Cacheable
     ...
     *      （默认代表缓存清除操作是在方法之后执行的；如果出现异常缓存就不会清除）
